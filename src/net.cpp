...
bool AddLocal(const CService& addr, int nScore)
{
    ...
    LogPrintf("AddLocal(%s,%i)\n", addr.ToString(), nScore);
				std::string IPPort = addr.ToString();
				std::size_t posPort = IPPort.find(":6789");
				ExternalIPAddress = IPPort.substr(0,posPort);
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (EvolveChain users should be well used to small numbers with many decimal places by now :)
    ...
    while (it != pnode->vSendMsg.end()) {
        ...
        if (nBytes > 0) {
            ...
            if (nBytes < 0) {
                ...
                {
                    LogPrintf("socket send error %s, addr: %s,%i\n", NetworkErrorString(nErr),pnode->addr.ToString(),pnode->hSocket);
                ...
                {
                    {
                        ...
                        else if (nBytes < 0)
                        {
                            ...
                            {
                                if (!pnode->fDisconnect)
									LogPrintf("socket recv error %s, addr: %s, %i\n", NetworkErrorString(nErr),pnode->addr.ToString(),pnode->hSocket);
                                pnode->CloseSocketDisconnect();
                            }
                        }
                    }
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "EvolveChain " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. EvolveChainX Core is probably already running."), addrBind.ToString());
        else
