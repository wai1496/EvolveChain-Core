...
{
    CEvolveChainAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered evolvechain address / amount:
            ...
            scriptPubKey.SetDestination(CEvolveChainAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal evolvechain:URI (evolvechain:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CEvolveChainAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CEvolveChainAddress(address).ToString().c_str()].push_back(out);
    }
}
...
{
    CTxDestination dest = CEvolveChainAddress(sAddress).Get();
