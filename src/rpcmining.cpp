...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or evolvechain.conf setting gen)\n"
	...
	if(identiCode!=""){
		...
		if (fGenerate && Params().NetworkID() == CChainParams::REGTEST)
		{
			...
			while (nHeight < nHeightEnd)
			{
				...
				{
					nHeightLast = nHeight;
					GenerateEvolveChains(fGenerate, pwalletMain, 1);
				}
				MilliSleep(1);
				{   // Don't keep cs_main locked
					LOCK(cs_main);
		...
		else // Not -regtest: start generate thread, return immediately
		{
			...
			GenerateEvolveChains(fGenerate, pwalletMain, nGenProcLimit);
		}

		return Value::null;
	}else{
		return "setgenerate Error: You are not identified. Get yourself identified to generate coins.\nYou can use Settings->Options->Identification Tab->Start Verification\n\nYou can also use RPC command startverification\n";
	}	
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "EvolveChain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "EvolveChain is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.evolvechain.it/wiki/BIP_0022 for full specification.\n"
