...
*Setup instructions for a gitian build of EvolveChain using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the EvolveChain
...
- [Building EvolveChain](#building-evolvechain)
...
```bash
sudo apt-get install git ruby sudo apt-cacher-ng qemu-utils debootstrap lxc python-cheetah parted kpartx bridge-utils gcc python-dev zip
adduser debian sudo
```
...
Clone the git repositories for evolvechain and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/GreenCoin-Project/EvolveChain-Core
```
...
In [doc/release-process.md](release-process.md) in the evolvechain repository under 'Fetch and build inputs'.
...
Building EvolveChain
----------------

To build EvolveChain (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the evolvechain repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/evolvechain/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/evolvechain/evolvechain
...
For example:
```bash
URL=https://github.com/laanwj/evolvechain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit evolvechain=${COMMIT} --url evolvechain=${URL} ../evolvechain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit evolvechain=${COMMIT} --url evolvechain=${URL} ../evolvechain/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/evolvechain-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/evolvechain-build.assert
```
...
[evolvechain/gitian.sigs](https://github.com/evolvechain/gitian.sigs/) repository, or
